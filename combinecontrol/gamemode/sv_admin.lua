...
local function GiveOddity( ply, args )
	...
	if( targ and targ:IsValid() and item != "" ) then
		
		if ( GAMEMODE:GetOddityByID( item ) ) then
		
			targ:GiveOddity( item );
		
			GAMEMODE:LogAdmin( "[M] " .. ply:Nick() .. " gave " .. targ:RPName() .. " the " .. GAMEMODE:GetOddityByID( item ).Name .. " oddity.", ply );
		
		else
		
			net.Start( "nAddChat" );
				net.WriteVector( Vector( 200, 0, 0 ) );
				net.WriteString( "That isn't a valid oddity." );
			net.Send( ply );
...
concommand.AddAdmin( "rpa_giveoddity", GiveOddity );

local function RemoveOddity( ply, args )
	...
	if( targ and targ:IsValid() and item != "" ) then
		...
		targ:RemoveOddity( targ:GetOddity( item ) );
		
		GAMEMODE:LogAdmin( "[M] " .. ply:Nick() .. " removed an oddity from " .. targ:RPName() .. ".", ply );
...
concommand.AddAdmin( "rpa_removeoddity", RemoveOddity );

local function ListOddities( ply, args )
	
	local item = args[1] or "";
	
	if( item == "" ) then
		
		net.Start( "nAListOddities" );
			net.WriteString( "" );
		net.Send( ply );
		return;
...
concommand.AddAdmin( "rpa_listoddities", ListOddities );

local function CheckOddities( ply, args )
	...
	if( #args == 0 ) then
		...
		net.Start( "nACheckOddities" );
			net.WriteTable( targ.Oddities );
		net.Send( ply );
	
	end
	
end
concommand.AddAdmin( "rpa_checkoddities", CheckOddities );
...
local function SetToolTrust( ply, args )
	...
	if( trust != 0 and trust != 1 and trust != 2 and trust != 3 ) then
...
local function SetDonatorRank( ply, args )
	...
	if( trust != 0 and trust != 1 and trust != 2 and trust != 3 ) then
		...
		targ:SetDonatorRank( trust );
		
		GAMEMODE:LogAdmin( "[S] " .. ply:Nick() .. " changed player " .. targ:RPName() .. "'s SetDonatoRank to \"" .. tostring( trust ) .. "\".", ply );
		
		local rf = { ply, targ };
	
		targ:UpdatePlayerField( "DonatorRank", trust );
		
		
	
end
concommand.AddAdmin( "rpa_setdonatorrank", SetDonatorRank );
...
for k, v in pairs( GM.Stats ) do
	...
	local function f( ply, args )
		...
		if( targ and targ:IsValid() ) then
			...
			local val = math.Clamp( add + targ[v]( targ ), -1000, 1000 );
