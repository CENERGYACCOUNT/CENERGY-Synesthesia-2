...
function meta:FriendlyActiveCPs()
	if self:NPCHatesFlaggedCPs() then
		return false
	end
...
function meta:FriendlyInactiveCPs()
	if self:NPCHatesUnflaggedCPs() then
		return false
	end
...
function GM:RefreshNPCRelationship(ent)
	...
	for _, v in pairs(player.GetAll()) do
		...
		if ent:HateUnflaggedCPs() then
			if v:isCombine() or v.HasOddity ("rolecultist") then
				hated = true
			end
		end
		...
		if hated then
			...
			if self.DefaultNPCRelationships[ent:GetClass()] then
				...
				if v:isCombine() and not v:HasOddity ("rolecultist") then
					if t[1] == 1 then
						ent:AddEntityRelationship(v, D_HT, 99)
					end
				...
				elseif v.HasOddity ("rolecultist") then
					if t[2] == 1 then
						ent:AddEntityRelationship(v, D_HT, 99)
					end
				...
				elseif not v.HasOddity ("rolecultist") then
					if t[4] == 1 then
						ent:AddEntityRelationship(v, D_HT, 99)
					end
...
function GM:OnNPCKilled(npc, attacker, inflictor)
	...
	if npc:GetClass() == "npc_cenergy_rat" then
		if math.random(1, 1) == 1 then
			GAMEMODE:CreatePhysicalItem("meat", npc:GetPos(), Angle())
		end
	end
	
	if npc:GetClass() == "npc_cenergy_alienspider" then
		if math.random(1, 1) == 1 then
			GAMEMODE:CreatePhysicalItem("meat", npc:GetPos(), Angle())
		end
	end

	if npc:GetClass() == "npc_cenergy_cyborg" then
		if math.random(1, 1) == 1 then
			GAMEMODE:CreatePhysicalItem("iron", npc:GetPos(), Angle())
		end
	end

	if npc:GetClass() == "npc_cenergy_spider" then
		if math.random(1, 1) == 1 then
			GAMEMODE:CreatePhysicalItem("meat", npc:GetPos(), Angle())
		end
	end
	...
	if npc:GetClass() == "npc_cenergy_spacecadoman" then
		if math.random(1, 1) == 1 then
			GAMEMODE:CreatePhysicalItem("spacecado", npc:GetPos(), Angle())
				GAMEMODE:CreatePhysicalItem("spacecado", npc:GetPos(), Angle())
...
function meta:GoTo(pos, style)
	...
	if ent:GetClass() == "npc_helicopter" or ent:GetClass() == "npc_combinegunship" or ent:GetClass() == "npc_combinedropship" then
		...
		if not targ1 or not targ1:IsValid() then
		...
		if not targ2 or not targ2:IsValid() then
