...
function nASetDonatorRank( len )
	...
	if( LocalPlayer() == ply ) then
		...
		GAMEMODE:AddChat( Color( 229, 201, 98, 255 ), "CombineControl.ChatNormal", "You set " .. targ:RPName() .. "'s DonatorRank to " .. str .. ".", { CB_ALL, CB_OOC } );
...
net.Receive( "nASetDonatorRank", nASetDonatorRank );
...
function nAListOddities( len )
	
	local filter = net.ReadString();
	
	if( filter == "" ) then
		chat.OldAddText( Color( 128, 128, 128, 255 ), "ODDITY LIST:" );
	else
		chat.OldAddText( Color( 128, 128, 128, 255 ), "ODDITY LIST (FILTER \"" .. filter .. "\"):" );
	end
	
	for _, v in pairs( GAMEMODE.Oddity ) do
...
net.Receive( "nAListOddities", nAListOddities );

function nACheckOddities( len )
	
	local odd = net.ReadTable();
	
	for _, v in pairs( odd ) do
				
		chat.OldAddText( Color( 229, 201, 98, 255 ), GAMEMODE:GetOddityByID( v ).ID, "\t", Color( 128, 128, 128, 255 ), GAMEMODE:GetOddityByID( v ).Name, "\n", GAMEMODE:GetOddityByID( v ).Description );
		
	end
	
end
net.Receive( "nACheckOddities", nACheckOddities );
