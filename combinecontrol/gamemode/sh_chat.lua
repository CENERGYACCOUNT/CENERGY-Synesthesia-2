...
if( SERVER ) then
	...
	function nChangeRadio( len, ply )
		...
		local radio = "radio" or "combineradio"
	...
	function nChangeCombineRadio( len, ply )
		
		local FLAG_CP = GAMEMODE:LookupCombineFlag( ply:ActiveFlag() ) or GAMEMODE:LookupCombineFlag( ply:CombineFlag() )
		...
		local CHAR_FLAGS = FLAG_COTA or FLAG_CIVIL or FLAG_DNA
		
		if( !OKAY ) then return end
		
		if( !ply:HasItem( "combineradio" ) ) then return end
	...
	net.Receive( "nChangeCombineRadio", nChangeCombineRadio );
...
function GM:ChatLocal( ply, arg )
	...
	if( ply:HasOddity("injuryheavy3") ) then
		
		if( CLIENT ) then
			
			GAMEMODE:AddChat( Color( 200, 0, 0, 255 ), "CombineControl.ChatNormal", "Your vocal cordes are damaged. You cannot speak.", { CB_ALL, CB_IC } );
...
function CC.CanRadio( ply )
	...
	elseif( ply:HasItem( "combineradio" ) ) then
...
function CC.CheatRoll( ply, arg, val )
...
GM:AddChatCommand( "/ScottSterling", Color( 200, 200, 200, 255 ), CC.CheatRoll );
...
CC.Languages = { };
CC.Languages[TRAIT_CONDUCTOR] = { "Turkish", "/tur" };
CC.Languages[TRAIT_ROCKY] = { "Bengali", "/ben" };
CC.Languages[TRAIT_LOYALIST] = { "Portugese", "/por" };
CC.Languages[TRAIT_GLUTTON] = { "Arabic", "/ara" };
CC.Languages[TRAIT_LUCKY] = { "Hindi", "/hin" };
CC.Languages[TRAIT_MULE] = { "Czech", "/cze" };
CC.Languages[TRAIT_SPY] = { "Sign Language", "/sig" };
CC.Languages[TRAIT_SPEEDY] = { "Vortigese", "/vor" };
CC.Languages[TRAIT_GREASE] = { "Polish", "/pol" };
...
function CC.AAT( ply, arg, col )
	...
	if( SERVER ) then
		...
		GAMEMODE:AddChat( col, "CombineControl.ChatNormal", arg, { CB_ALL, CB_IC }, nil );
		
	end
	
end

function CC.AATBig( ply, arg, col )
	...
	if( SERVER ) then
		...
		GAMEMODE:AddChat( col, "CombineControl.ChatBig", arg, { CB_ALL, CB_IC }, nil );
