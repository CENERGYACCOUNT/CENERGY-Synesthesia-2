	...
	if( self.DrugType ) then
		...
		if( self.DrugType == DRUG_VODKA ) then
			...
			if( d <= 60 ) then
				...
				tab[ "$pp_colour_mulr" ] 		= 3000 * mul;
				tab[ "$pp_colour_mulg" ] 		= 0 * mul;
				tab[ "$pp_colour_mulb" ] 		= 0 * mul;
...
function GM:DrugEffectMedkit()
	...
	surface.PlaySound( Sound( "npc/zombie_poison/pz_call1.wav" ) );
...
function GM:DrugEffectVodka()
	...
	self.DrugAmbience = CreateSound( LocalPlayer(), "npc/zombie_poison/pz_breathe_loop2.wav" );
	self.DrugAmbience:SetSoundLevel( 0 );
	self.DrugAmbience:Play();
