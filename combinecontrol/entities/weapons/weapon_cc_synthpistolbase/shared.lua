...
if( CLIENT ) then
   ...
   SWEP.LaserMat = Material( "effects/blueblacklargebeam" );
...
function SWEP:PreDrawViewModel( vm, ply, wep )
   ...
   if( !self.Owner:Holstered() and self.IronMode < IRON_AIM and self.IronMode >= IRON_IDLE and self:GetNextPrimaryFire() <= CurTime() and self:Clip1() > 0 ) then
      ...
      render.DrawSprite( self.Owner:GetEyeTrace().HitPos, 4, 4, Color( 50, 190, 225, 255 ) );
...
function SWEP:AddViewKick()
    ...
    if self.Owner:HasOddity("skillakimborecoilcontrol") then
        recoil = recoil * 0.5 -- Halve recoil if skill is present
    end
