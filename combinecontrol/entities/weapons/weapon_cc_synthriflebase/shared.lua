...
function SWEP:PrimaryUnholstered()
    if (GAMEMODE:Combat() == 0) or (self.Owner:Special() > self.Primary.AttackCostS - 0.1) then
        local canUseSpecialInstead = false
        
        if (GAMEMODE:Combat() == 0) or (self.Owner:Attack() > self.Primary.AttackCostA - 0.1) then
            -- Normal Attack Point Check Passed
        elseif self.Owner:HasOddity("abilityflexible") and not self.Owner:HasOddity("cooldownflexible") then
            -- Use Special Instead if Attack Points Are Insufficient and Ability is Present
            canUseSpecialInstead = true
        else
            return -- Not enough Attack points and cannot use Special as a substitute
        end
        ...
        if (GAMEMODE:Combat() == 0) or (self.Owner:Movement() > self.Primary.AttackCostM - 0.1) then
            if self:CanPrimaryAttack() then
                ...
                if IsFirstTimePredicted() then
                    if self.Owner:HasOddity("skillakimborecoilcontrol") then
                        self:ShootBullet(self.Primary.Damage, self.Primary.Force, self.Primary.NumBullets, self.Primary.Accuracy * 0.5 * self:BulletAccuracyModifier())
                    else
                ...
                if SERVER then
                    timer.Simple(0, function()
                        if canUseSpecialInstead then
                            self.Owner:SetSpecial(math.Clamp(self.Owner:Special() - self.Primary.AttackCostA, 0, 100))
                            self.Owner:GiveOddity("cooldownflexible") -- Apply Cooldown Oddity
                        else
...
-- Adjust CrippleChance if the attacker has "statusheadhunting"
local crippleChance = self.CrippleChance
if self.Owner:HasOddity("statuskneeshooting") then
    crippleChance = crippleChance + 25
end

if math.random(1, 100) <= crippleChance then
        ...
        -- Check if penetration succeeds
         if effectivePenetration >= armorValue then
            if hitgroup == HITGROUP_HEAD then
            	    ...
            	    if self.Owner:HasOddity("statusheadhunting") then
       effectiveDamage = effectiveDamage * 2.5
    else
                effectiveDamage = effectiveDamage * 2 end
            elseif hitgroup == HITGROUP_LEFTARM or hitgroup == HITGROUP_RIGHTARM then
