...
function SWEP:PrimaryUnholstered()
    local specialCost = self.Primary.AttackCostS
    local attackCost = self.Primary.AttackCostA
    local movementCost = self.Primary.AttackCostM

    if self.Owner:HasOddity("statussprayandpray") then
        specialCost = specialCost * 0.5
        attackCost = attackCost * 0.5
        movementCost = movementCost * 0.5
    end

    local canFire = (GAMEMODE:Combat() == 0) or (self.Owner:Special() > specialCost - 0.1)
    local hasAttack = (GAMEMODE:Combat() == 0) or (self.Owner:Attack() > attackCost - 0.1)
    local hasMovement = (GAMEMODE:Combat() == 0) or (self.Owner:Movement() > movementCost - 0.1)

    if canFire and (hasAttack or self.Owner:HasOddity("statusoverheat")) and hasMovement then
        -- Ability logic here
        if self:CanPrimaryAttack() then
            ...
            if IsFirstTimePredicted() then
                if self.Owner:HasOddity("statussprayandpray") then
                    self:ShootBullet(self.Primary.Damage, self.Primary.Force, self.Primary.NumBullets, self.Primary.Accuracy * 2 * self:BulletAccuracyModifier())
                else
            ...
            if SERVER then
                timer.Simple(0, function()
                    if IsValid(self.Owner) then
                        local finalSpecialCost = self.Owner:HasOddity("statussprayandpray") and self.Primary.AttackCostS * 0.5 or self.Primary.AttackCostS
                        local finalAttackCost = self.Owner:HasOddity("statussprayandpray") and self.Primary.AttackCostA * 0.5 or self.Primary.AttackCostA
                        local finalMovementCost = self.Owner:HasOddity("statussprayandpray") and self.Primary.AttackCostM * 0.5 or self.Primary.AttackCostM

                        -- Apply cost reductions
                        self.Owner:SetSpecial(math.Clamp(self.Owner:Special() - finalSpecialCost, 0, 100))
                        self.Owner:SetMovement(math.Clamp(self.Owner:Movement() - finalMovementCost, 0, 100))

                        if hasAttack then
                            -- Normal Attack Cost
                            self.Owner:SetAttack(math.Clamp(self.Owner:Attack() - finalAttackCost, 0, 100))
                        elseif self.Owner:HasOddity("statusoverheat") then
                            -- If out of Attack, damage the player instead
                            local damageAmount = finalAttackCost * 5
                            local dmg = DamageInfo()
                            dmg:SetDamage(damageAmount)
                            dmg:SetAttacker(self.Owner)
                            dmg:SetInflictor(self)
                            dmg:SetDamageType(DMG_BURN) -- Treat as overheating/burning damage
...
local SupressionChance = self.SupressionChance
if self.Owner:HasOddity("statuspindown") then
    SupressionChance = SupressionChance + 50
end
...
-- Check if the target has the 'statusmarktarget' oddity
    ...
    tr.Entity:TakeDamage(effectiveDamage, ply, self)
            ...
            tr.Entity:SetVelocity(shotDirection * (force * 0.00000001))
