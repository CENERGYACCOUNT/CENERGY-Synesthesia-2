...
ENT.Contact			= "";
ENT.Purpose			= "To craft items via other items.";
ENT.Instructions	= "";
...
function ENT:Think()
	...
	if( self:GetStoveOn() ) then
		...
		if( self:GetModel() == "models/props_wasteland/controlroom_desk001b.mdl" ) then
		...
		for _, v in pairs( GAMEMODE.RecipesCT ) do
		...
		if( self.ExplodeTime and CurTime() > self.ExplodeTime and self:GetModel() != "models/props_wasteland/controlroom_desk001b.mdl" ) then
...
function ENT:Use( ply )
	...
	if( SERVER ) then
		...
		if( ( !self.SteamEnt or !self.SteamEnt:IsValid() ) and self:GetModel() != "models/props_wasteland/controlroom_desk001b.mdl" ) then
		
			local pos = self:GetPos();
			if( string.lower( self:GetModel() ) == "models/props_wasteland/controlroom_desk001b.mdl" ) then
		...
		if( self:GetStoveOn() ) then
			...
			self:EmitSound( Sound( "physics/metal/metal_barrel_impact_soft1.wav" ) );
...
function ENT:Draw()
	...
	if( !self:GetInvisible() ) then
		...
		if( self:GetStoveOn() and self:GetModel() == "models/props_wasteland/controlroom_desk001b.mdl" ) then
			
			render.SetMaterial( self.LightMat );
			render.DrawSprite( self:GetPos() + self:GetForward() * -16 + self:GetRight() * -54 + self:GetUp() * 12, 8, 8, Color( 255, 255, 200, 255 ), false );
